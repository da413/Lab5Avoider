...
public class AvoiderEngineTestScript : MonoBehaviour
{
    ...
    private IEnumerator AvoidanceLoop()
    {
        while (true)
        {
            if (avoidee != null)
            {
                if (AvoideeCanSeeAvoider(avoidee.transform))
                {
                    FindCandidateSpots();
    ...
    private void FindCandidateSpots()
    {
        ...
        foreach (Vector2 sample in sampler.Samples())
        {
            ...
            if (!PointIsVisibleToAvoidee(worldPoint))
            {
    ...
    private bool AvoideeCanSeeAvoider(Transform target)
    {
    ...
    private bool PointIsVisibleToAvoidee(Vector3 point)
    {
